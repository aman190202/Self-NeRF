...
def train():
    """Trains our NeRF on the LLFF dataset."""
    ...
    class Args:
        def __init__(self, config):
            self.__dict__.update(config['training_parameters'])
            self.__dict__.update(config['model_parameters'])
            self.__dict__.update(config['dataset_parameters'])
            self.__dict__.update(config['rendering_parameters'])
            self.__dict__.update(config['logging_parameters'])
            self.__dict__.update(config['device_parameters'])

    # Load data
    K = None

    images, poses, bds, render_poses, i_test = load_llff_data(args.dataset_path, 8,
                                                                recenter=True, bd_factor=.75,
                                                                spherify="store_true")
    hwf = poses[0,:3,-1]
    poses = poses[:,:3,:4]
    print('Loaded llff', images.shape, render_poses.shape, hwf, args.output_dir)
    ...
    os.makedirs(os.path.join(args.output_dir, args.dataset_path), exist_ok=True)
